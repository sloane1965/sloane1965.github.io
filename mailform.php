...
$subject = "Work with me";$emailaddress = "oldmansloane@gmail.com"; /* NOTE: Although your email address is visible here in this code, the person contacting you will never see this email address. Your email address will remain on your server, and it will not be sent from your server to the person contacting you. It will also remain invisible to spam bots. Your email address	is also never stored on any of our servers. You can choose to delete or not delete this note when you publish this page. It will not change the functionality of the contact form. */
...
function checkpriority($field,$checkrequirepriority) {
	...
	$name = $_REQUEST['name'];strlencheck($name,1,60,"You have not entered a proper name. Please hit your browser back button and check your name entry.","You have entered a name that is too long. Please hit your browser back button and check your name entry.");
$companyname = $_REQUEST['companyname'];strlencheck($companyname,0,60,"You have not entered a proper company name. Please hit your browser back button and check your name entry.","You have entered a company name that is too long. Please hit your browser back button and check your entry.");$phone = $_REQUEST['phone'];checkphone($phone,0,"Improper first phone number detected. Please hit your browser back button and try again."); $phone2 = $_REQUEST['phone2'];checkphone($phone2,0,"Improper second phone number detected. Please hit your browser back button and try again."); $priority = $_REQUEST['priority'];checkpriority($priority,1); if ($dontsendemail == 0) $dontsendemail = spamcheck($_REQUEST["htest"]);
...
if ($dontsendemail == 0) $dontsendemail = spamcheck($phone2);
if ($dontsendemail == 0) $dontsendemail = spamcheck($priority);
...
if ($dontsendemail == 0) {
	...
	$message.="Second phone number: ".$phone2."\r\n";
